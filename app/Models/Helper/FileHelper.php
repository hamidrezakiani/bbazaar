...
class FileHelper
{
    ...
    public static function uploadToLocalFilament($file, $prefix, $thumb = true)
    {
        try {
            ...
            if ($thumb) {
                self::generateThumbLocalFilament($file, $filename);
            }
    ...
    public static function generateVideoThumbnail($file, $prefix)
    {
        try {
            ...
            $fileN = $prefix . '-thumb-' . time() . '-' . mt_rand(1, 9) . '.jpg';

            $videoThumbnail = new VideoThumbnail();
            $videoThumbnail->createThumbnail(
                public_path('uploads/'.$filename),
                public_path('uploads/thumb/'),
                $fileN,
                2,
                320,
                320);
    ...
    public static function generateThumbLocalFilament($file, $filename)
    {
        try {
            return ImageManager::imagick()->read($file)
                ->resize(320, 320, function ($constraint) {
                    $constraint->aspectRatio();
                })
                ->save(Storage::disk('public')
                        ->path('') . DIRECTORY_SEPARATOR . env('THUMB_PREFIX') . $filename, 80);
        } catch (\Exception $ex) {
            throw $ex;
        }
